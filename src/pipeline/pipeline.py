...
from datetime import datetime
import pandas as pd
from src.constant import EXPERIMENT_DIR_NAME, EXPERIMENT_FILE_NAME, CURRENT_TIME_STAMP
...
class Pipeline(Thread):
    ...
    def __init__(self, config: Configuartion ) -> None:
        try:
            ...
            Pipeline.experiment_file_path=os.path.join(config.training_pipeline_config.artifact_dir,EXPERIMENT_DIR_NAME,CURRENT_TIME_STAMP, EXPERIMENT_FILE_NAME)
    ...
    def start_model_trainer(self, data_transformation_artifact: DataTransformationArtifact) -> ModelTrainerArtifact:
        try:
            ...
            initialize_model_training = model_trainer.initiate_model_trainer()
            #model training completed in pipeline start model trainer
            
            self.model_training_status =  model_trainer.model_training_status
            print("model training status in pipeline/start_model_trainer: %s" % self.model_training_status)
            


            return initialize_model_training
        except Exception as e:
            raise spamhamException(e, sys) from e
    ...
    def is_pipeline_running(self):
        ...
        return self.pipeline_running_status

    
    @is_pipeline_running.setter
    def is_pipeline_running(self, pipeline_running_status):
        self.pipeline_running_status = pipeline_running_status
    ...
    def run_pipeline(self):
        try:
            ...
            self.pipeline_running_status = False
            print("model training status in pipeline/run_pipeline: %s" % self.model_training_status)
